...
| `cannot register: key directory … is not writable` | The connector refuses to register when it can't persist the key, so it doesn't waste the one-time token. This means the `/data` volume isn't writable by the container's non-root user (uid 65532). A **fresh** named volume with a current image is writable automatically; if you reuse a volume created by an older image (or a bind mount), fix its ownership once: `docker run --rm -v mkonnect-data:/data alpine chown -R 65532:65532 /data`, then restart. |
